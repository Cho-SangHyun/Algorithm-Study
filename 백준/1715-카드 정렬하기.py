...
# 가정 : 처음에 다른 조합를 포함하는 최적해가 있던데?
# 반박 1) : 10, 20, 40의 조합이 있다고 하면 10 20부터 골라간 게 정답이고 이보다 작은 경우는 없다.
# 반박 2) 
#     매 선택마다 제일 작은 카드묶음 2개를 골라 합쳐나갔을 때의 비교횟수 집합을
#     G = {g1, g2, g3, ..., gn} 이라 하고, 이 문제의 정답의 비교횟수 집합을
#     S = {s1, s2, s3, ..., sn} 이라 한다. 
#     이 때 문제의 가정에 따라 G와 S는 다른 집합이므로 SUM(m)을 1항부터 m항까지의 합이라고 하면 
#     G의 SUM(m)과 S의 SUM(m)이 달라지는 1이상의 자연수 m이 무조건 존재한다. 그리고 G는 늘 제일 작은
#     더미를 골라나간 것이므로 이 때 G의 SUM(m) < S의 SUM(m)이 성립. 
#     새로운 집합 NS = {g1, g2, g3, ..., gm, s(m+1), s(m+2), ..., sn} 을 만들면
#     이 NS의 총 비교횟수가 S의 총 비교횟수보다 작아야 하지만, 문제의 가정에서 S가 최적해라고 했으므로
#     NS가 S보다 작은 값을 내는 상황 자체가 모순이 됨. 즉 제일 작은 카드들부터 골라간 G가 최적해 
